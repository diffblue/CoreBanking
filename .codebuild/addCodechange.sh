...
if [ $(git config --get remote.origin.url)"" == "git@github.com:diffblue/CoreBanking.git" ]; then
	if [ $(git name-rev --name-only HEAD)"" == "dcover-notrivial-asserts-demo" ]; then
		if [ -f "src/main/java/io/diffblue/corebanking/account/Account.java" ]; then
			...
			google-chrome https://github.com/diffblue/CoreBanking/compare/dcover-notrivial-asserts-demo...$branchName?expand=1
		else
